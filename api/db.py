...
def create_tables_if_not_exist(conn):
    """
    Create Categories and Audits tables if they don't exist,
    and insert default entries for categories and their respective tests.
    """
    cur = conn.cursor()
    ...
    CREATE TABLE IF NOT EXISTS Systems (
        ...
        systemmac VARCHAR(255) NOT NULL,
  ...
  # Create GroupSystems table
    create_groupsystems_table = """
  CREATE TABLE IF NOT EXISTS GroupSystems (
      groupid INT,
      systemid INT,
  ...
  """
    ...
    cur.execute(create_audits_table)
    cur.execute(create_systems_table)
    cur.execute(create_groups_table)
    cur.execute(create_groupsystems_table)

    # Default categories and tests (audits)
    default_data = [
  {
  ...
  },
  {
    ...
    # Insert default data into Categories and Audits tables
    for entry in default_data:
        ...
        SELECT %s WHERE NOT EXISTS (SELECT 1 FROM Categories WHERE categoryname = %s);
        """, (category_name, category_name))

        # Get the CategoryID of the inserted/found category
        cur.execute("SELECT categoryid FROM Categories WHERE categoryname = %s;", (category_name,))
        category_id = cur.fetchone()[0]

        # Insert the tests (audits) related to the category
        for test in entry["tests"]:
            cur.execute("""
            INSERT INTO Audits (auditname, categoryid)
            SELECT %s, %s WHERE NOT EXISTS (SELECT 1 FROM Audits WHERE auditname = %s AND categoryid = %s);
...
def get_db_connection():
    """
    Create and return a new database connection and ensure tables exist.
    """
    ...
    # Ensure the tables are created if they don't already exist
